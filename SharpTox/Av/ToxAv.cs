...
namespace SharpTox.Av
{
    ...
    public class ToxAv : IDisposable
    {
        ...
        private readonly List<ToxAvDelegates.GroupAudioReceiveCallback> _groupAudioHandlers = new List<ToxAvDelegates.GroupAudioReceiveCallback>();

        private ToxAvHandle _toxAv;

        private volatile bool _disposed = false;
        private volatile bool _running = false;
        private volatile AutoResetEvent _cancelEvent;
        private volatile AutoResetEvent _canceledEvent;
        ...
        public void Start()
        {
            ...
            _canceledEvent = new AutoResetEvent(false);
            new Thread(this.Loop) { IsBackground = true }.Start();
        }
        ...
        private void Loop()
        {
            ...
            while (!_disposed)
            {
                try
                {
                    if (_cancelEvent == null || _cancelEvent.WaitOne(0))
                    {
                        break;
                    }

                    int delay = this.DoIterate();
