    ...
    public class WaveFormatConversionProvider : IWaveProvider, IDisposable
    {
        ...
        public int Read(byte[] buffer, int offset, int count)
        {
            ...
            while (bytesRead < count)
            {
                ...
                {
                    NAudioLogger.Instance.LogWarning(string.Format("Warning: couldn't convert anything from {0}", sourceBytesAvailable));
                ...
                {
                    // possible error here
                    NAudioLogger.Instance.LogWarning(string.Format("sourceBytesRead: {0}, sourceBytesConverted {1}, destBytesConverted {2}", 
